        ...
        async function fetchAll() {
            ...
            try {
                ...
                if (res.data.status === 0) {
                    ...
                    // pre-select sub-service IDs returned from API
                    const selectedServiceIds = servicesData.map(s => s.id)
...
async function handleSave() {
    ...
    try {
        ...
        fd.append("local_area", form.local_area)
        ...
        form.services.forEach(id => {
            const sub = allSubServices.find(ss => ss.id === id)
            if (sub?.title) {
